...
export default class Members extends Component {
  ...
  inputValidation = async () => {
    ...
    if (memberImage.trim() == '' || memberImage.trim() == null) {
      Toast.show('Please select a image');
    } else if (name.trim() == '' || relation.trim() == '' || mobnum.trim() == '') {
  ...
  addMemberData = async () => {
    ...
    const { name, relation, mobnum, memberbase64Image, token } = this.state;
    this.setState({ isHidden: true });
    console.log('Image ==> ' + JSON.stringify(this.state.memberImage) + ' -- ' + this.state.memberbase64Image);

    console.log('input ==> ' + token + ' ' + name + ' ' + relation + ' ' + mobnum + ApiName.registerMember);

    axios
      .post(
        ApiName.registerMember,
        {
          name: name,
